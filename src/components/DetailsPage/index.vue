		...
		<div class="overview w1200 flex-space-between">
			...
			<div class="overview-content">
				<h2>
					...
					<span style="font-size: 12px; float: right;color: #808080;"> 您的当前位置：首页>>>{{ $route.query.newBannerText }}>>>{{ $route.query.newBanner }}>>> 新闻内容 </span>
				</h2>
				...
				<div class="overview-details" v-else v-for="(item, index) in synopsisHtmlContent" :key="index">
					<h3>{{ item.title }}</h3>
					<h4>
...
export default {
	...
	created() {
		this.request(this.$route.query.id, this.$route.query.data, this.$route.query.type);
	},
	mounted() {},
	methods: {
		typeface(type) {
			...
			switch (parseInt(type)) {
				case 1:
					WebApi.getNewsDetails(id, val)
						// WebApi.getNewsDetailsPage2(id, val)
						.then(({ data }) => {
							console.log(data);
